...
def load_ancient_data():
    ...
    try:
        ...
        res2 = db.table("ancient_scripts").select("*").order("created_at").execute()
        scripts = []
        for r in (res2.data or []):
            scripts.append({
                "id": r["script_num"],
        ...
        st.warning(f"Could not load ancient data: {e}")
        return empty


def save_ancient_script(script_record: dict):
    try:
        db = _client()
        if not db:
            return
        db.table("ancient_scripts").insert({
            "script_num": script_record["id"],
            "protocol": script_record.get("protocol", ""),
            "script_text": script_record.get("script", ""),
            "anchor": script_record.get("anchor", ""),
        ...
        st.error(f"Failed to save ancient script: {e}")


def load_ancient_fingerprints(limit: int = 15) -> list:
    try:
        db = _client()
        if not db:
            return []
        res = db.table("ancient_scripts").select(
            "anchor, pov, constraint_rule, word_target, tone, protocol, created_at"
        ).order("created_at", desc=True).limit(limit).execute()
...
def load_ancient_channel() -> dict:
    try:
        db = _client()
        if not db:
            return {}
        res = db.table("ancient_channel_settings").select("*").limit(1).execute()
        if res.data:
            return res.data[0]
        return {}
    except Exception:
...
def save_ancient_channel(channel_id: str, channel_url: str, channel_name: str):
    try:
        db = _client()
        if not db:
            return
        db.table("ancient_channel_settings").upsert({
            "id": 1,
            "channel_id": channel_id,
            "channel_url": channel_url,
            "channel_name": channel_name,
        ...
        st.error(f"Failed to save ancient channel: {e}")
